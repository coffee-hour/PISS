...
const Sovereign = (() => {
    // --- State ---
    let state = {
        resources: { food: 100, wood: 100, ore: 50, gold: 50, pop: 100 },
    ...
    const config = {
        ...
        buildings: {
            ...
            market: { id: 'market', name: "Global Exchange", res: "gold", baseCost: { wood: 50, gold: 20 }, prod: 0.5, icon: 'landmark' }
        },
        units: {
    ...
    const update = (delta) => {
        ...
        // Population growth logic: +1% per second if food > population, else -0.5% per second
        const popGrowthRate = state.resources.food > state.pop ? 0.01 : -0.005;
        state.pop = Math.max(10, state.pop + (state.pop * popGrowthRate * delta));
    ...
    const renderResourcesOnly = () => {
        ...
        // Update Subjects (Population) display
        const popPill = document.getElementById('res-pop');
        if (popPill) {
            const valEl = popPill.querySelector('.res-val');
            if (valEl) valEl.innerText = Math.floor(state.pop).toLocaleString();
        }
    };
    ...
    const prestige = () => {
        ...
        state.resources = { food: 100, wood: 100, ore: 50, gold: 50, pop: 100 };
