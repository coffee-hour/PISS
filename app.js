/**
 * Sovereign: Statecraft - Sovereign Engine (AAA V4 - Restoration)
 */
...
const Sovereign = (() => {
    ...
    const renderLedgerOnly = (gen = null) => {
        ...
        const popEl = document.querySelector('#res-pop .res-val');
        if (popEl) popEl.innerText = Math.floor(state.pop || 0).toLocaleString();
    };
    ...
    const renderCouncil = () => {
        ...
        if (!container) return;
        container.innerHTML = '';
        config.councilMembers.forEach(m => {
            const div = document.createElement('div');
            div.className = 'chamber-card iron-banded';
            div.innerHTML = `
                <div class="card-header"><h3><i data-lucide="${m.icon}" class="icon-stone"></i> ${m.name}</h3></div>
                <div class="card-body"><p>${m.desc}</p>
                <button class="action-btn candle-glow" onclick="Sovereign.notify('Minister ${m.name} assigned.')">Appoint Minister</button></div>`;
            container.appendChild(div);
        });
    };
    ...
    const load = () => {
        ...
        if (s) {
            try {
                ...
                console.error("State corruption detected. Resetting.");
            }
        }
        if (isNaN(state.pop) || state.pop === undefined) state.pop = 100;
    };
