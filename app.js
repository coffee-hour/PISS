/**
 * Sovereign: Viltrumite Combat (v2.7.2)
 * Bug Fix for Cursor and Boss Rendering.
 */
...
const Sovereign = (() => {
    ...
    const render = () => {
        ...
        if (g) g.innerHTML = config.upgrades.map(u => {
            const cost = Math.floor(u.baseCost * Math.pow(1.15, state.upgrades[u.id]||0));
            return `<div class="upgrade-card ${state.treasury < cost ? 'disabled' : ''}" onclick="Sovereign.buyUpgrade('${u.id}')"><div><i data-lucide="${u.icon}"></i></div><div style="flex:1"><b>${u.name}</b><br>${cost.toLocaleString()} Essence</div><div>${state.upgrades[u.id]||0}</div></div>`;
        }).join('');
        if (a) a.innerHTML = config.attributes.map(attr => {
            const cost = Math.floor(attr.baseCost * Math.pow(5, state.attributes[attr.id]));
            return `<div class="upgrade-card ${state.treasury < cost ? 'disabled' : ''}" onclick="Sovereign.buyAttr('${attr.id}')"><div><i data-lucide="${attr.icon}"></i></div><div style="flex:1"><b>${attr.name}</b><br>${cost.toLocaleString()} Essence</div><div>${state.attributes[attr.id]}</div></div>`;
        }).join('');
        const boss = config.bosses[state.bossIndex % config.bosses.length];
    ...
    const renderUI = () => {
        const tEl = document.getElementById('treasury-count');
        if (tEl) tEl.innerText = Math.floor(state.treasury).toLocaleString();
        const cEl = document.getElementById('combo-fill');
        if (cEl) cEl.style.width = (state.comboValue * 10) + '%';
        const boss = config.bosses[state.bossIndex % config.bosses.length];
        const maxBoss = boss.health * Math.pow(1.5, Math.floor(state.bossIndex / config.bosses.length));
        const bnEl = document.getElementById('boss-name');
        if (bnEl) bnEl.innerText = boss.name;
        const bhEl = document.getElementById('boss-health-fill');
        if (bhEl) bhEl.style.width = Math.max(0, (state.bossHealth / maxBoss) * 100) + '%';
        const phEl = document.getElementById('player-health-fill');
        if (phEl) {
            const maxPlayer = 100 + state.attributes.durability * 25;
            phEl.style.width = Math.max(0, (state.playerHP / maxPlayer) * 100) + '%';
        }
    };
    ...
    const save = () => localStorage.setItem('sov_combat_v272', JSON.stringify(state));
    const load = () => { try { const s = localStorage.getItem('sov_combat_v272') || localStorage.getItem('sov_combat_v271'); if (s) state = { ...state, ...JSON.parse(s), lastUpdate: Date.now() }; } catch(e) {} };
